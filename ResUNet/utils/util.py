...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    # image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy() #灰度image为（h，w）
    image, label = image.cpu().detach().numpy(), label.cpu().detach().numpy()  # 灰度image为（h，w）
